            ...
            ping.promise.probe(currentIp, { timeout: 1 }).then(async (res) => {
                if (res.alive) {
                    arp.getMAC(res.inputHost, async (err, mac) => {
                        const manufacture = mac ? await getManufacture(mac).catch(_ => "Unknown") : "Unknown";
...
let networks;
let timestamp;
async function getCachedResultsAsync() {
    if (!networks || timestamp + 1000 < Date.now()) {
        networks = await scanNetwork();
        timestamp = Date.now();
    }
    return networks
}
...
module.exports = { getCachedResults, getCachedResultsAsync }